<template>
	<view class="transfer clearfix lh1">
		...
		<form class="block clearfix">
			...
			<button @click="formValidate" class="h45 f15 flex-text bg-theme color-black mx20 mt30">{{ i18n.confirm }}</button>
		</form>
	</view>
</template>
...
export default{
	...
	methods: {
		...
		async formSubmit(){
			...
			await this.$http.post('wallet.transfer', params, { tipConfig: { isSuccessDefaultTip: true } })
			.then(res=> {
