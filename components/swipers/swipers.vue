<template>
	...
	>
		<swiper-item :key="i" @click="_handleClick(slide)" class="slider-item" v-for="(slide, i) of slides" v-if="!useSlot">
			<view class="hp100 flex align-center" v-if="text">
				<text class="ellipsis lh1 text">{{ slide[srcName] || slide }}</text>
			</view>
			<image :mode="mode" mode="widthFix" :src="baseUrl + (slide[srcName] || slide)" class="slider-img" :style="imgStyle" v-else></image>
		</swiper-item>
		<slot></slot>
	</swiper>
</template>
