...
let Pattern = {
	...
	isPassword: /^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{6,16}$/,
	isPayPassword: /^[0-9]{6}$/,
	isAuthCode: /^[0-9]{4}$/,
	isEmail: /^\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/,
...
const validator = {
	 ...
	 * @returns {boolean}
	 */
	isPayPassword: function(strValue) {
		if (validator.isEmpty(strValue)) {
			return false
		}
		return validator.executeExp(Pattern.isPayPassword, strValue)
	},
	/**
	 * 判断是否符合密码规则
	 * @param strValue 校验的值
	 ...
	 * @returns {boolean}
	 */
	isEth: function(strValue) {
		if (validator.isEmpty(strValue)) {
			return false
		}
	
		return validator.executeExp(Pattern.isEth, strValue)
	},
