 ...
 * Request 1.0.5
 * @Class Request
 * @description luch-request 1.0.5 http请求插件
 * @Author lu-ch
 * @Date 2019-12-12
...
export default class Request {
		...
		return new Promise((resolve, reject) => {
			...
			options.complete = (response) => {
				...
				if (this.validateStatus(response.statusCode)) { // 成功
					response = this.requestComFun(response, { resolve, reject })
					resolve(response)
				} else {
					response = this.requestComFail(response, { resolve, reject })
					reject(response)
				}
			}
	...
	connect(url, data, options = {}) {
		return this.request({ url, data, method: 'CONNECT', ...options })
	}
	// #endif

	// #ifdef APP-PLUS || H5 || MP-WEIXIN || MP-BAIDU
	head(url, data, options = {}) {
		return this.request({ url, data, method: 'HEAD', ...options })
	}
	// #endif

	// #ifdef APP-PLUS || H5 || MP-WEIXIN || MP-BAIDU
	options(url, data, options = {}) {
		return this.request({ url, data, method: 'OPTIONS', ...options })
	}
	// #endif

	// #ifdef APP-PLUS || H5 || MP-WEIXIN
	trace(url, data, options = {}) {
		return this.request({ url, data, method: 'TRACE', ...options })
	}
	// #endif
	...
	upload(url, {
		...
		return new Promise((resolve, reject) => {
			let next = true
			let handleRe = {}
			const globalHeader = { ...this.config.header, ...config.header || {} }
			...
			const pubConfig = {
				...
				formData: formData || {},
				method: 'UPLOAD',
				// header: config.header || this.config.header,
				header: globalHeader,
